...
module xilinx_sync_fifo #(parameter FIFO_DEPTH = 512,FIFO_WIDTH = 8,FIFO_MODE = "FWFT")(
    clk         ,
    rst_n       ,
...
//函数声明
    function integer clog2b(input integer data);begin
        data = data - 1;
        for(clog2b = 0;data > 0;clog2b = clog2b + 1)
            data = data >> 1;
    end 
    endfunction  

//参数声明
    localparam  COUNT_WIDTH = clog2b(FIFO_DEPTH) + 1                ,
                READ_MODE   = FIFO_MODE == "FWFT" ? "fwft" : "std"  ,
                READ_LATEN  = FIFO_MODE == "FWFT" ? 0 : 1           ;
//信号定义
    input                       clk         ;
    input                       rst_n       ;
    ...
    output  [COUNT_WIDTH-1:0]   wr_count    ; 
    output                      al_full     ;
    output                      full        ;
    output                      wr_ack      ;
    ...
    output                      rd_data_vld ;
    output  [COUNT_WIDTH-1:0]   rd_count    ;
    output                      al_empty    ;
    output                      empty       ;

    assign al_full = wr_count >= FIFO_DEPTH - 8;
    assign al_empty = rd_count <= 8;
   ...
   xpm_fifo_sync #(
      ...
      .FIFO_READ_LATENCY(READ_LATEN),     // DECIMAL
   ...
   xpm_fifo_sync_inst (
      ...
      .data_valid(rd_data_vld   ),       // 1-bit output: Read Data Valid: When asserted, this signal indicates
